...
class GraphView: UIView {
    ...
    init(frame: CGRect, data: [[Double: Int]]) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for i in 0...5 {
//            print(yLabelInterval)
            let label = axisLabel(title: String(format: "%.1f", Double(i) * Double(yLabelInterval)/10.0) + " ºC")
            
            label.frame = CGRect(x: -5, y: floor((rect.size.height - padding) - CGFloat(i) * (axisHeight / 5) - 10), width: 25, height: 30)
            addSubview(label)
        ...
        let xLabel = axisLabel(title: "Day Hour Min")
        xLabel.frame = CGRect(x: 0, y: graphHeight + 30, width: 40, height: 50)
        xLabel.textAlignment = .center
        xLabel.textRect(forBounds: xLabel.frame, limitedToNumberOfLines: 3)
        addSubview(xLabel)
    ...
    func plotPoint(point : [Double: Int], path: CGMutablePath) {
        ...
        let date = Date(timeIntervalSince1970: point.keys.first!)
        ...
        let xLabel = axisLabel(title: strDate)
        xLabel.frame = CGRect(x: xposition - 10, y: graphHeight + 30, width: 20, height: 50)
        xLabel.textAlignment = .center
        xLabel.textRect(forBounds: xLabel.frame, limitedToNumberOfLines: 3)
        addSubview(xLabel)
